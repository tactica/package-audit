...
module Package
  class TestAudit < Minitest::Test
    ...
    def test_that_there_is_an_error_message_when_a_file_is_not_found
      output = `bundle exec package-audit outdated test/files/not_found.lock`
      assert_match 'No such file or directory @ rb_sysopen - test/files/not_found.lock', output
    end

    def test_that_there_is_an_error_message_when_a_file_is_invalid
      output = `bundle exec package-audit outdated test/files/invalid.lock`
      assert_match 'NoMethodError', output
    end

    def test_that_there_is_a_success_message_when_everything_is_up_to_date
      output = `bundle exec package-audit outdated test/files/empty_gemfile.lock`
      assert_match 'Bundle up to date!', output
    end

    def test_that_headers_are_correct_and_present
      output = `bundle exec package-audit outdated test/files/gemfile1.lock`
      lines = output.split(/\n/)
    ...
    def test_that_only_explicit_gems_are_displayed
      output = `bundle exec package-audit outdated test/files/gemfile1.lock --only-explicit`
      lines = output.split(/\n/)
    ...
    def test_that_gems_are_shown_in_csv_format_with_headers
      output = `bundle exec package-audit outdated test/files/gemfile1.lock --only-explicit --csv`
      lines = output.split(/\n/)
    ...
    def test_that_gems_are_shown_in_csv_format_without_headers
      output = `bundle exec package-audit outdated test/files/gemfile1.lock --only-explicit --csv --exclude-headers`
      lines = output.split(/\n/)
